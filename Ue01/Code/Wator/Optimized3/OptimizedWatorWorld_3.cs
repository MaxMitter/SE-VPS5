...
namespace VPS.Wator.Optimized3 {
    // initial object-oriented implementation of the Wator world simulation
    public class OptimizedWatorWorld_3 : IWatorWorld {
        ...
        public OptimizedWatorWorld_3(Settings settings) {
            ...
            for (int i = 0; i < Width; i++) {
                for (int j = 0; j < Height; j++) {
                    int value = randomMatrix[j * Width + i];
                    if (value < InitialFishPopulation) {
                        Grid[j * Width + i] = new Fish(this, new Point(i, j), random.Next(0, FishBreedTime));
                    } else if (value < InitialFishPopulation + InitialSharkPopulation) {
                        Grid[j * Width + i] = new Shark(this, new Point(i, j), random.Next(0, SharkBreedEnergy));
                    } else {
                        Grid[j * Width + i] = null;
                    }
                }
        ...
        public void ExecuteStep() {
            ...
            for (int i = 0; i < Width; i++) {
                for (int j = 0; j < Height; j++) {
                    ...
                    col = randomMatrix[j * Width + i] % Width;
                    row = randomMatrix[j * Width + i] / Width;

                    if (Grid[row * Width + col] != null && !Grid[row * Width + col].Moved)  // process unmoved animals
                        Grid[row * Width + col].ExecuteStep();
                }
            }
            ...
            for (int i = 0; i < Width; i++) {
                for (int j = 0; j < Height; j++) {
                    if (Grid[j * Width + i] != null)
                        Grid[j * Width + i].Commit();
                }
            }
        }
        ...
        public Bitmap GenerateImage() {
            ...
            for (int y = 0; y < Height; y++) {
                for (int x = 0; x < Width; x++) {
                    Color col;
                    if (Grid[y * Width + x] == null) col = Color.DarkBlue;
                    else col = Grid[y * Width + x].Color;
        ...
        public Point[] GetNeighbors(Type type, Point position) {
            ...
            if ((type == null) && (Grid[j * Width + i] == null)) {
                ...
                if ((Grid[j * Width + i] != null) && (!Grid[j * Width + i].Moved)) {  // ignore animals moved in the current iteration
                    neighbors[neighborIndex] = new Point(i, j);
                    neighborIndex++;
                }
            }
